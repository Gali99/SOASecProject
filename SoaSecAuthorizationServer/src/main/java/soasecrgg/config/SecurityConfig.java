...
public class SecurityConfig {
    ...
    public SecurityFilterChain asSecurityFilterChain(HttpSecurity http) throws Exception {
        ...
        OAuth2AuthorizationServerConfiguration.applyDefaultSecurity(http); //Applica le impostazioni di sicurezza predefinite per un server di autenticazione OAuth 2.0
        ...
        return http
                ...
                .oauth2ResourceServer(OAuth2ResourceServerConfigurer::jwt) //Configura l'applicazione per funzionare come server di risorse OAuth 2.0 che protegge le sue risorse utilizzando token JWT per l'autorizzazione.
                .build();
    }
    ...
    public SecurityFilterChain appSecurityFilterChain(HttpSecurity http) throws Exception {
        return http
                .formLogin(withDefaults()) //Abilita l'autenticazione tramite form login con impostazioni predefinite. Modulo di login HTML standard per consentire agli utenti di inserire le proprie credenziali (username e password).
                .authorizeHttpRequests(authorize ->authorize.anyRequest().authenticated()) //solo gli utenti autenticati potranno accedere alle risorse dell'applicazione protette da questa catena di filtri.
                .build();
    ...
    public RegisteredClientRepository registeredClientRepository() {
        RegisteredClient registeredClient = RegisteredClient.withId(UUID.randomUUID().toString()) //imposta un ID univoco per il client utilizzando UUID.
                .clientId("client") //imposta l'id client
                .clientSecret(passwordEncoder().encode("secret")) //Imposta il segreto client, utilizzato per l'autenticazione del client. Codificato con il passwordEncoder
                .scope("read")
                .scope(OidcScopes.OPENID) //Aggiunge lo scope OpenID, che consente l'autenticazione basata su OpenID Connect.
                .scope(OidcScopes.PROFILE)
                .redirectUri("http://127.0.0.1:8080/login/oauth2/code/myoauth2") //Imposta l'URI di redirect a cui l'utente verrà reindirizzato dopo l'autorizzazione.
                .clientAuthenticationMethod(ClientAuthenticationMethod.CLIENT_SECRET_BASIC) //Utilizza il segreto client per l'autenticazione.
                .authorizationGrantType(AuthorizationGrantType.AUTHORIZATION_CODE)
                .authorizationGrantType(AuthorizationGrantType.REFRESH_TOKEN) //Consente l'utilizzo di refresh token per ottenere nuovi token di accesso.
    ...
    OAuth2TokenCustomizer<JwtEncodingContext> tokenCustomizer() {
        return context -> {
            Authentication principal = context.getPrincipal(); //Oggetto che contiene informazioni sull'utente autenticato.
            Set<String> authorities = principal.getAuthorities().stream()
                    .map(GrantedAuthority::getAuthority).collect(Collectors.toSet()); //Estrae le autorità dell'utente dall'oggetto Authentication.
            if (context.getTokenType().getValue().equals("id_token")) { //Controlla il tipo di token che viene generato (access token o ID token).
    ...
    /**
     * Questo metodo statico fornisce un modo semplice per generare una nuova coppia di chiavi RSA
     * @return Dopo la generazione riuscita della chiave, il metodo restituisce l'oggetto keyPair generato.
     */
